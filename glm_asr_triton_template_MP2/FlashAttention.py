...
# ============================================================================
# Mixed Precison Kernel
# ============================================================================

@triton.jit
def flash_attention_kernel_mixed(
    q_ptr, k_ptr, v_ptr, output_ptr,
    scale,
    seq_q, seq_k, head_dim,
    stride_q0, stride_q1, stride_q2,
    ...
    m = tl.full((BLOCK_Q,), float("-inf"), dtype=tl.float32)
    l = tl.zeros((BLOCK_Q,), dtype=tl.float32)
    acc = tl.zeros((BLOCK_Q, BLOCK_D), dtype=tl.float32)  # FP32 累积
    ...
    for k_block in range(num_k_blocks):
        ...
        exp_scores = tl.exp(scores - m_new[:, None]).to(tl.float32)
        l = l + tl.sum(exp_scores, axis=1)
        ...
        # FP16×FP16 → FP32 累积
        acc = acc + tl.dot(exp_scores.to(tl.float16), v, allow_tf32=True)
