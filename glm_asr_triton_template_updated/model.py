...
class GlmAsrModel:
    ...
    def generate(
        ...
        # Session-level dtype normalization: cast once before autoregressive loop
        # to avoid repeated per-step conversions.
        decoder_compute_dtype = self.lm_head.weight.dtype
        if inputs_embeds.dtype != decoder_compute_dtype:
            inputs_embeds = inputs_embeds.to(decoder_compute_dtype)
        ...
        # Prefill once and initialize KV cache
        logits, past_key_values = self.decode(inputs_embeds=inputs_embeds, use_cache=True)

        # Autoregressive generation (incremental decode with KV cache)
        for _ in range(max_new_tokens):
            ...
            # Incremental step: decode only the newly generated token
