...
def rmsnorm_kernel(
    ...
    *** TODO: Implement this kernel ***
    ...
    Grid: (batch_size,)
    Load FP16 -> Cast to FP32 -> Compute -> Store FP16。
    """
    pid = tl.program_id(0)
...
class RMSNorm:
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_triton and x.is_cuda:
            ...
            # 修改点：删除 .to(torch.float32)，直接使用 x_flat (FP16)
...
def gelu(x: torch.Tensor) -> torch.Tensor:
    ...
    x_flat = x.reshape(-1).contiguous()# 修改点：.to(torch.float32)
    output = torch.empty_like(x_flat)
...
class Linear:
    ...
    def _ensure_weight_prepared(self):
        ...
        if self._weight_t_padded is None:
            ...
            if self._K_padded > K or self._N_padded > N:
                ...
                weight_pad = torch.zeros(
                    (self._K_padded, self._N_padded),
                    dtype=weight_t.dtype, # <--- 改为保持原权重的 dtype
                    device=weight_t.device,
                )
    ...
    def _forward_triton(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 权重也建议转为 FP16 (如果权重本身是 FP32)
        #if self.weight.device != x.device:
        #    self.weight = self.weight.to(x.device)
        #
        # 【修复】：必须同时检查设备 和 数据类型
        # 权重必须移动到 GPU (x.device) 并且转为 FP16 (x.dtype)
        if self.weight.device != x.device or self.weight.dtype != x.dtype:
            self.weight = self.weight.to(device=x.device, dtype=x.dtype)
            self._weight_t_padded = None # 清空缓存，以便重新生成 GPU 上的 Pad 后权重
        ...
        #)
        # 输出 buffer 设为输入类型
        output = torch.zeros((M_padded, self._N_padded), dtype=x.dtype, device=x.device)
