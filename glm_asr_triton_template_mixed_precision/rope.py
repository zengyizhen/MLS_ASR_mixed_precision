...
def _apply_rope_single(
    ...
    cos_expanded = cos[None, None, :, :].to(torch.float32)# 修改点
    sin_expanded = sin[None, None, :, :].to(torch.float32)# 修改点
    ...
    # 转回原数据类型 (FP16)
    return torch.cat([x1_rot.to(x.dtype), x2_rot.to(x.dtype)], dim=-1)
