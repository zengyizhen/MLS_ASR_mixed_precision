...
def softmax_inplace_kernel(scores_ptr, stride_s, seq_k, BLOCK_SIZE: tl.constexpr):
    ...
    s = tl.load(scores_ptr + row * stride_s + offs, mask=mask, other=-float("inf")).to(tl.float32) # 转为 FP32
    s = s - tl.max(s, axis=0)
    exp_s = tl.exp(s)
...
def scaled_dot_product_attention(
    ...
    if use_triton:
        # 修改点：移除 .to(torch.float32)，保持 FP16
        ...
        if seq_k_padded != seq_k or head_dim_padded != head_dim:
            ...
            v_flat = v_padded
            q_flat = q_padded
        # Scores 矩阵通常需要 FP32 来防止 Softmax 溢出
        # 但为了显存优化，也可以用 FP16 存储，Kernel 内部转 FP32 计算
        # 建议保持 scores 为 float32 以获得最高精度，或者 fp16 + upcast。
        # 这里为了性能，我们可以尝试让 kernel 处理转换。
        scores = torch.empty(
        ...
        output = torch.empty(
            (batch * num_heads, seq_q, head_dim_padded),
            dtype=q.dtype,#torch.float32, # 输出恢复为 FP16
            device=q.device,
        )
